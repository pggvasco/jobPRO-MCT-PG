Public Class xfCancelJobStage
    ...
    Private Sub xfCancelJobStage_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            Dim CRlm As New Helper.ActiveRecord.LookupMaintenance(Of API.CancelReasonCode)(CancelReasonCodeLookUpEdit, xaeCancelReasonCode, AddressOf LoadCancelReasonCode, Me.Conversation)
        Catch ex As Exception
            Setup.Log.logError("Error setting lookup maintenance for Work Centre Categories.", ex)
        End Try
    End Sub

    Private Function LoadCancelReasonCode() As IEnumerable(Of API.CancelReasonCode)
        Return API.CancelReasonCode.FindAll
    End Function
    ...
    Private Function TestInfo()
        ...
        '        Try
        '            FieldPrompt = Job.CancelJobStage.FieldPrompts.Where(Function(num) num.FieldName = BaseEditControl.Name.Substring(4)).First
        '        Catch
        ...
        If CancelReasonCodeLookUpEdit.EditValue IsNot Nothing Then
            If CType(CancelReasonCodeLookUpEdit.EditValue, API.CancelReasonCode).IsForceCommentValue = True AndAlso (CancelComentTextEdit.Text Is Nothing OrElse CancelComentTextEdit.EditValue = "") Then
                CancelComentTextEdit.ErrorText = "Please specify a Cancel Comment."
                check = False
            End If
        Else
            CancelReasonCodeLookUpEdit.ErrorText = "Please specify a Cancel Reason."
            check = False
        End If
    ...
    Private Sub CancelReasonCodeLookUpEdit_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles CancelReasonCodeLookUpEdit.EditValueChanged
        If CancelReasonCodeLookUpEdit.EditValue IsNot Nothing Then
            If CType(CancelReasonCodeLookUpEdit.EditValue, API.CancelReasonCode).IsCommentAllowed Then
                CancelComentTextEdit.Properties.ReadOnly = False
            Else
                CancelComentTextEdit.Properties.ReadOnly = True
            End If
        End If
    End Sub
End Class