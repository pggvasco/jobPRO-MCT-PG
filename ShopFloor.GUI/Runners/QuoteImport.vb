Public Class QuoteImport
    ...
    Public Property QuoteToImport As API.Job
    ...
    Public Sub Process()
        If QuoteToImport Is Nothing Then Throw New ArgumentNullException("QuoteToImport", "No Quote specified to import from.")
        If Job Is Nothing Then Throw New ArgumentNullException("Job", "No Job specified to import to.")

        Dim objects As New List(Of API.JobLine)(QuoteToImport.JobLines)
        ...
        If QuoteToImport.Drawing IsNot Nothing Then
            If Job.Drawing Is Nothing Then
                Job.Drawing = QuoteToImport.Drawing
            End If
            If Job.Drawing = QuoteToImport.Drawing Then
                For Each jtida As API.JobDrawingAnnotation In QuoteToImport.JobDrawingAnnotations
                    If Job.JobDrawingAnnotations.Count > jtida.Sequence - 1 Then
        ...
        Job.ImportReference = "Quote Number : " + QuoteToImport.QuoteNumber
        'Job.SaveAndFlush()
    End Sub
