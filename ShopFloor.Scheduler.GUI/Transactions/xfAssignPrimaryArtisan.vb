...
Public Class xfAssignPrimaryArtisan
...
#Region "Load Events"
    ...
    Public Sub SetCLH()
        ...
        CLU.Components.Add(lcAssignJob)
        CLU.Components.Add(gvAssignJob)
    End Sub

    Private Sub xfAssignJob_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetIcons()
...
#End Region
    ...
    Public Function GetSelectedPrimaryArtisan() As API.Artisan
        ...
        job = CType(gvAssignJob.GetFocusedRow, API.Job)
...
#Region "Grid Events and Validation"
    ...
    Private Sub gvAssignJob_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gvAssignJob.DoubleClick
        Dim artisan As API.Artisan
        Dim frm As New xfAssignedArtisanOperations

        Dim m As MouseEventArgs = e
        Dim x As DevExpress.XtraGrid.Views.Grid.ViewInfo.GridHitInfo = gvAssignJob.CalcHitInfo(m.Location)
        ...
        If x.InRow Then
            If Not gvAssignJob.IsGroupRow(x.RowHandle) Then
                Try
    ...
    Private Sub gvAssignJob_InvalidRowException(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.InvalidRowExceptionEventArgs) Handles gvAssignJob.InvalidRowException
        e.ExceptionMode = DevExpress.XtraEditors.Controls.ExceptionMode.NoAction
        Beep()
    End Sub

    Private Sub gvAssignJob_ValidateRow(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs) Handles gvAssignJob.ValidateRow
        ...
        If job IsNot Nothing Then
            If job.PrimaryArtisan Is Nothing Then
                ...
                gvAssignJob.SetColumnError(colArtisan, "Please select the Primary Artisan.")
            End If

            If job.RequiredDeliveryDate Is Nothing Then
                e.Valid = False
                e.ErrorText = "Please select the Required Delivery Date."
                gvAssignJob.SetColumnError(colRequiredDeliveryDate, "Please select the Required Delivery Date.")
            End If
    ...
    Private Sub xfAssignPrimaryArtisan_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        gvAssignJob.CloseEditor()

        If gvAssignJob.HasColumnErrors Then
...
#Region "Refresh Grid"
    ...
    Private Sub beiAutoRefreshRate_EditValueChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles beiAutoRefreshRate.EditValueChanged
...
#End Region
    ...
    Private Sub btDrawing_ButtonClick(sender As System.Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles btDrawing.ButtonClick
        Dim job As API.Job = TryCast(gvAssignJob.GetFocusedRow, API.Job)
    ...
    Private Sub gvAssignJob_ShowingEditor(sender As System.Object, e As System.ComponentModel.CancelEventArgs) Handles gvAssignJob.ShowingEditor
        If gvAssignJob.FocusedColumn IsNot colDrawing Then Exit Sub
        Dim job As API.Job = TryCast(gvAssignJob.GetFocusedRow, API.Job)
